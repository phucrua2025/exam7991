...
export const StatsChart: React.FC<StatsChartProps> = ({ plans }) => {
  const data = React.useMemo(() => {
    const counts = { NB: 0, TH: 0, VD: 0 };
    plans.forEach(p => {
      // Safely check if level exists in our counts (ignoring old VDC if present in stale data)
      if (p.level in counts) {
        counts[p.level as keyof typeof counts] += p.count;
      }
    });
    return [
